...
<form class="update-form" action="/<?="<?=\$this->route?>"?>update?<?=$generator->genKeyForModel($model, 'model')?>&backUrl=<?="<?=urlencode(\$backUrl)?>"?>" method="post">
	<h1><a class="list" href="<?="<?=\$backUrl ?: '/' . \$this->route?>"?>"><?=$title?></a> &gt; <a href="/<?="<?=\$this->route?>"?>update?<?=$generator->genKeyForModel($model, 'model')?>&backUrl=<?="<?=urlencode(\$backUrl)?>"?>">编辑: <?='<?="{$model->'.implode('} - {\$model->', $model::priKeys()).'}"?>'?></a></h1>
