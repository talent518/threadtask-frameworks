...
use app\modules\backend\models\UserSearch as Search;
 ...
 */
class UserController extends \fwe\base\Controller {
	use TplView;
	
	public function actionIndex(RequestEvent $request, int $page = 1, int $size = 10, string $orderBy = 'uid', bool $isDesc = true) {
		$db = db()->pop();
		$model = Search::create();
				...
				if($row === null) {
					...
					$row->save(
						...
						function(\stdClass $status) use($request, $row, $backUrl) {
							$request->getResponse()->redirect("/{$this->route}view?id={$row->uid}&backUrl=" . urlencode($backUrl));
						},
						function(\stdClass $status) use($request, $row, $backUrl) {
